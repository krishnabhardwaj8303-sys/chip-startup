module masked_sbox(
    ...
    // Standard S-Box table (existing hai)
    ...
    // ── MASKING LOGIC ──
    // Step 1: Input ko mask ke saath XOR karo
    wire [7:0] masked_input;
    assign masked_input = data_in ^ mask_in;

    // Step 2: Masked lookup table use karo
    // (Real hardware mein yeh precomputed 
    //  masked table hoti hai — yahan hum 
    //  unmask karke, sbox lookup karke,
    //  phir naya mask apply kar rahe hain — 
    //  simplified but demonstrates the 
    ...
    always @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            // Actual S-box computation 
            // (unmask -> lookup -> remask)
            sbox_result <= sbox_lookup(data_in);

            // Naya random mask generate karo 
            // (real chip mein TRNG se aayega)
            new_mask <= mask_in ^ 8'hA5; // Simplified rotation

            // Masked output — power trace 
            // ab random mask ke saath 
            // correlate hoga, actual data se nahi
            data_out <= sbox_result ^ new_mask;
            mask_out <= new_mask;
        end
    end
endmodule
